...
describe("Traversing DOM elements in Cypress", () => {
  beforeEach(() => {
    ...
    it("first() to retrieve the first DOM element within elements ", () => {
      cy.get('.traversal-table >tbody > tr > td').first().then((el) => {
        el.text()
        cy.log(el.text())
        expect(el.text()).to.equal('Andy')
      })
      
    });
  
    it("last() to retrieve the last DOM element within elements", () => {
      cy.get('.traversal-table >tbody > tr > td').last().then((el) => {
        el.text()
        cy.log(el.text())
        expect(el.text()).to.equal('Scott')
      })
    ...
    it("parents() to get parents DOM element of elements", () => {
      cy.get('.traversal-cite').parents('blockquote').should('match', 'blockquote')
    });
